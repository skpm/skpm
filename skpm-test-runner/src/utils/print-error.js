  ...
  stack.forEach(frame => {
    if (
      ...
      if (
        ...
        // ignore generated files and our frames
        return
      }
      console.log(
        `    ${chalk.dim(
          `at ${mappedPosition.name || '<anonymous>'} (`
        )}${chalk.blue(
          path.relative(process.cwd(), mappedPosition.source)
        )}${chalk.dim(`:${mappedPosition.line}:${mappedPosition.column})`)}`
      )
    } else {
      // ignore the rest, it's not actionable
    }
  })
}
