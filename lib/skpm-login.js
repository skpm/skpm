...
var spinner = ora({text: 'Checking if the token is valid', color: 'yellow'}).start()
github.getUser(program.token).then(function () {
  spinner.text = 'Token valid'
  spinner.succeed()
  spinner = ora({text: 'Saving access token to the keychain', color: 'yellow'}).start()
  spinner.color = 'yellow'

  auth.saveToken(program.token)

  spinner.text = 'Access token saved to the keychain'
  spinner.succeed()
  process.exit(0)
}).catch(function (err) {
