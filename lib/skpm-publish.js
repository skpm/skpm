...
.then(function (res) {
  ...
  return new Promise(function (resolve, reject) {
    ...
    appcastObj.rss.channel[0].item.unshift({
      enclosure: [{
        $: {
          url: 'https://github.com/' + repo + '/releases/download/' + tag + '/' + path.basename(packageJSON.skpm.main) + '.zip',
...
.then(function (res) {
  ...
  return exec('zip -r ' + tempZip + ' ' + packageJSON.skpm.main + ' -x "*.DS_Store"')
})
.then(function (res) {
...
.then(function (res) {
  ...
  return github.updateAsset(token, repo, releaseId, tempZip, path.basename(packageJSON.skpm.main) + '.zip')
})
.then(function () {
