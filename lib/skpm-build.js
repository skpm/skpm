...
function rollupConfig (file, command) {
  const _config = {
    ...
    plugins: (userDefinedRollupConfig.plugins || []).concat([
      babel(),
      nodeResolve({
        jsnext: true,
        main: true
...
var counter = 0
var steps = manifestJSON.commands.length + (packageJSON.resources || []).length + 1
function checkEnd () {
  if (++counter >= steps) {
...
manifestJSON.commands.concat(packageJSON.resources || []).forEach(function (command) {
