...
function copyManifest () {
  return new Promise(function (resolve, reject) {
    ...
    if (manifestJSON.appcast !== false && packageJSON.appcast !== false) {
      copy.appcast = manifestJSON.appcast || packageJSON.appcast || defaultAppcastURL
    } else {
      delete copy.appcast
    }

    if (!copy.author && packageJSON.author) {
...
const commandsAndHandlers = manifestJSON.commands.reduce(function (prev, c) {
  if (!prev[c.script]) {
    prev[c.script] = {
      script: c.script,
      handlers: [],
  ...
  prev[c.script].handlers.push(c.handler || 'onRun')
  prev[c.script].identifiers.push(c.identifier)
  return prev
}, {})

var commands = Object.keys(commandsAndHandlers).map(function (k) {
