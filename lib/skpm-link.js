...
try {
  ...
  if (fs.existsSync(path.join(pluginDirectory, packageJSON.name, packageJSON.skpm.main))) {
    console.log(chalk.red('error') + ' This plugin has already been linked.')
    process.exit(0)
  }

  // Create the symlink within the encompassing directory
  fs.symlinkSync(getPath(packageJSON.skpm.main), path.join(pluginDirectory, packageJSON.name, packageJSON.skpm.main))
