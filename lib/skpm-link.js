...
var packageJSON
try {
  packageJSON = require(path.join(process.cwd(), program.path, 'package.json'))
} catch (err) {
...
try {
  ...
  fs.symlinkSync(path.join(pluginDirectory, packageJSON.name, packageJSON.main), path.join(process.cwd(), program.path, packageJSON.main))
