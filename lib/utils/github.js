  ...
  addPluginToPluginsRegistryRepo: function (token, packageJSON, repo) {
    ...
    .then(function (data) {
      ...
      return request(options(token, 'https://api.github.com/repos/sketchplugins/plugin-directory/forks', 'POST')).then(function (fork) {
        ...
        var compatibleSketchVersion
        if (typeof packageJSON.engines === 'string') {
          compatibleSketchVersion = packageJSON.engine
        } else if ((packageJSON.engines || {}).sketch) {
          compatibleSketchVersion = packageJSON.engine.sketch
        }
        var plugin = {
          title: packageJSON.name,
          name: name,
          owner: owner,
          appcastURL: 'https://raw.githubusercontent.com/' + repo + '/master/.appcast.xml',
          description: packageJSON.description,
          homepage: 'https://github.com/' + repo
        }
        if (compatibleSketchVersion) {
          plugin.compatibleSketchVersion = compatibleSketchVersion
        }
