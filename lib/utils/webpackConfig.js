...
var WebpackFooterPlugin = require('./WebpackFooterPlugin')
var SketchCommandPlugin = require('./sketchCommandPlugin')
var objectAssign = require('object-assign')
var config = require('../utils/config').get()
var semver = require('semver')

var footer = `var that = this;
function injectContext (fn) {
...
if (typeof exports === 'function') {
  that.onRun = injectContext(exports);
} else {
  Object.keys(exports).forEach(function (k) {
    if (k === 'default') {
      that.onRun = injectContext(exports[k]);
    } else {
      that[k] = injectContext(exports[k]);
    }
  })
}
`
  ...
  return function webpackConfig (file, commandIdentifier, sketchVersion) {
    ...
    if (commandIdentifier) {
      plugins.push(
        ...
        new WebpackFooterPlugin(footer)
      )
    }
    ...
    return objectAssign(
      ...
      userDefinedWebpackConfig,
      {
        ...
        output: {
          filename: basename,
          library: commandIdentifier ? 'exports' : undefined,
