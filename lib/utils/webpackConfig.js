...
function runCommand (output, commandIdentifier) {
  var command = `${config.sketchPath}/Contents/Resources/sketchtool/bin/sketchtool123 run "${output}" ${commandIdentifier}`
  ...
  var handleError = (
    ...
    'elif (echo "$res" | grep "no such file or directory: /Applications/Sketch.app"); then ' +
...
module.exports = function (program, output, manifestFolder, packageJSON) {
  ...
  return function webpackConfig (file, commandIdentifier) {
    ...
    if (commandIdentifier && program.run) {
      console.log(runCommand(output, commandIdentifier))
      plugins.push(
        new WebpackShellPlugin({
          onBuildEnd: [runCommand(output, commandIdentifier)],
          dev: false,
          safe: true
        })
      )
